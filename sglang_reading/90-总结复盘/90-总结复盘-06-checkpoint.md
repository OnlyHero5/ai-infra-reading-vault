...
- [ ] 能回答架构决策：何时 PD / 何时 spec / 何时依赖 RadixCache（见 [[90-总结复盘-02-设计追问与框架对比|90-总结复盘-02-设计追问与框架对比]]）

---

## 未独立成专题的主题

PP、HiCache、remote KV connector 等见 [[90-总结复盘-05-未独立成专题导读]]，按 30 分钟补课表阅读即可，无需等待新专题。
...
2. **torch.compile / connector 远程 KV** — 未单独成专题；见 [[90-总结复盘-05-未独立成专题导读]] 与 Attention/PD 交叉章节。
...
1. **导读层**可独立 onboarding：项目定位、架构分层、双协议七 hop、12 核心概念。
2. **可观测性/32** 补充生产运维向内容（可观测性、权重热更新）。
3. **专题深度**仍以阅读方法论–multimodal_gen 为主体；导读层负责串联与导航，复盘层负责查漏与自测。
