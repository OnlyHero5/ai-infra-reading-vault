...
**Explain：** 这里把“用户 API 的统一入口”和“解释器内部节点类型”分开。外部一个函数，内部两类 IR。
...
**Comment：** `gen_int`、`gen_string` 等语法糖本质上是给 `SglGen` 设置 dtype，真正转 regex 发生在 RuntimeEndpoint。
...
**Explain：** `run()` 是 IR 函数和解释器之间的入口。它不直接执行用户函数，而是把执行上下文整理好交给 interpreter。
...
**Comment：** `bind(**kwargs)` 的参数会在 `run_program` 里合并到 `func_kwargs`，这让函数可以预绑定部分输入。
...
**Explain：** 这是 Python 调用语义上的分流：同一个函数对象在不同上下文中可以是执行，也可以是 trace。
...
**Comment：** batch precache 用 tracing 提取公共前缀，正是依赖这个 `__call__` 分支。
...
**Explain：** `ProgramState` 是用户函数看到的 `s`，`StreamExecutor` 是真正执行 IR 的机器。
...
**Comment：** 这里的线程是 frontend runtime 线程，不是 SRT 后端 worker 线程。
...
**Explain：** 这让 `ProgramState += expr` 可以快速入队，实际执行由 worker 顺序处理，从而把用户 Python 控制流和后端请求解耦。
...
**Comment：** `contextvars.copy_context()` 用来把当前上下文传播到 worker，避免 tracing/logging 等上下文丢失。
...
**Explain：** 变量事件先建，可以让别的分支提前等待某个变量，而不因为生成尚未执行就找不到 event。
...
**Comment：** 这就是 `ProgramState.__iadd__` 背后的入口。
...
**Explain：** SGLang 前端没有先编译成 bytecode，而是运行时逐节点解释。这个分派表就是语言语义的核心。
...
**Comment：** 这里也能看出 backend capability 的边界：前端会判断能力，不是盲目调用后端高级接口。
...
**Explain：** 变量事件是前端语言能写 `s["x"]` 的关键。生成完成前变量可能还不可用，streaming 时变量会逐步增长。
...
**Comment：** `meta_info` 保存 logprob、token 等后端信息，是 debugging 和高级控制流的重要入口。
...
**Explain：** role 是前端语法糖，真正处理 prefix、suffix、messages 的是 `StreamExecutor._execute_role_begin/end`。
...
**Comment：** 这解释了为什么 API 层也有 `system/user/assistant` 函数：它们都是 role IR 的不同入口。
...
**Explain：** fork 是状态复制，不是重新运行前缀。支持 KV cache append 时，后续 join 还能让后端复用 fork 前缓存。
...
**Comment：** `position_ids_offset` 参数目前传入但未在复制逻辑中使用，读代码时不要假设它已经生效。
...
**Explain：** select 和 gen 的变量语义保持一致：都能生成可读变量，也都改变后续 prompt。
...
**Comment：** `choices_method` 抽象让不同选择归一化策略可插拔。
...
**Explain：** 这是 frontend 与 serving 后端的握手。没有 model_info，role begin/end 无法可靠生成聊天格式。
...
**Comment：** `support_concate_and_append=True` 也在初始化中声明，后续 fork join 会用到这个能力位。
...
**Explain：** 这是 IR 到 serving API 的转换层。Frontend 不直接访问 tokenizer 或 scheduler，只发后端兼容的 HTTP 请求。
...
**Comment：** 图片由 `_add_images` 插入，说明 multimodal 输入也被作为 executor 状态携带到后端请求。
...
**Explain：** dtype 是 frontend 便利语法，regex 是后端约束协议。转换放在 RuntimeEndpoint，避免 API 层绑定具体后端约束实现。
...
**Comment：** int/float 自动加入 stop，是为了让数字生成在常见分隔符处结束。
...
**Explain：** 这是 frontend batch precache 的后端动作：不生成新 token，只让后端处理并缓存 prefix。
...
**Comment：** `cache_program` 中有长度阈值，避免为短前缀额外发预热请求。
...
**Explain：** lazy commit 与 cache_prefix 类似，但输入来自当前 executor 状态，而不是外部传入的 prefix string。
...
**Comment：** 这段配合 `_execute_concatenate_and_append_kv_cache` 使用，属于高性能分支，不影响普通文本语义。
...
**Explain：** 前端 batch 并行主要是并发发起后端请求，不是 Python CPU 密集计算。线程池用于隐藏网络/后端等待。
...
**Comment：** 这里的线程数默认很大，说明设计更偏 IO 并发而不是 CPU 计算并行。
