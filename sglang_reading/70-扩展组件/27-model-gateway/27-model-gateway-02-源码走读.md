...
**Explain：** 这段代码是 gateway 的最外层协议表。它定义了哪些路径属于“受保护推理 API”，也解释了为什么后续 handler 看起来很薄：server 层只负责从 HTTP 世界进入内部 trait 世界。
...
**Comment：** 读这段时要抓住边界：`server.rs` 是协议门面，不是负载均衡器。真正的 gateway 设计从 handler 委托开始。
...
**Explain：** `/generate` 是最简单的委托样例。它不判断路由模式，只抽取 `body.model.as_deref()`，然后调用统一 trait 方法。
...
**Comment：** 读者可以把 server handler 看成 adapter：把 Axum 请求变成 `RouterTrait` 调用。
...
**Explain：** `create_worker` 是动态注册入口。它把 JSON 配置交给 `worker_service.create_worker`，成功或失败都转换成 HTTP response。
...
**Comment：** 这不是简单 CRUD。创建 worker 是把运行时 topology 写进 gateway 的路由数据结构。
...
**Explain：** 这段 match 是 gateway 路由形态的权威入口。它把配置空间收敛成少数实现类：HTTP Regular、HTTP PD、gRPC Regular、gRPC PD、OpenAI。
...
**Comment：** 工厂层回答的是“创建哪种路由器”，不是“请求去哪个 worker”。后者在 router 的热路径里发生。
...
**Explain：** 这段构造函数说明 HTTP Regular router 是“依赖注入对象”，不是资源拥有者。它拿到的是共享引用。
...
**Comment：** 这里的设计哲学是“路由器持有能力引用，不持有拓扑真相”。拓扑真相在 `WorkerRegistry`。
...
**Explain：** 这段是 HTTP Regular 路由的决策核心。`enable_igw=false` 时忽略 `model_id`，单 router 模式按全局 worker 池选；IGW 模式才按模型索引筛。
...
**Comment：** 这段把“谁有资格接请求”和“谁最适合接请求”拆开，是 gateway 能支持多策略的关键。
...
**Explain：** 这段构建逻辑说明 hash ring 是拓扑数据，不是请求临时数据。请求路径只做查找，注册/删除 worker 时才 rebuild。
...
**Comment：** 一致性哈希的设计目标不是“平均轮询”，而是“稳定地把相似请求黏在 cache 更可能命中的位置”。
...
**Explain：** 这段是 HTTP typed 请求的通用模板。真正的一次 worker 选择和发送发生在闭包里，因此每次 retry 都可以重新执行一次路由。
...
**Comment：** 这段体现 gateway 的生产取舍：请求代理不是裸转发，而是 metrics、retry、policy 重新选择的组合。
...
**Explain：** 这段与 generation 热路径形成对照：管理 GET 不走 policy，不用 hash ring，也不触发 typed request retry 模板。
...
**Comment：** Gateway 并不是所有 endpoint 都均衡转发。不同 endpoint 的一致性要求不同，代码用不同代理路径表达这一点。
...
**Explain：** 这段代码说明 DP rank 是 gateway 在代理层注入的请求字段，而不是用户显式传入的 API 参数。
...
**Comment：** 这部分读源码时要顺着函数调用继续看 `send_typed_request`，不要把 `route_typed_request` 误认为只做普通 HTTP 转发。
...
**Explain：** 这段注释非常关键：streaming 的成功条件不是 initial status，而是流干净结束。
...
**Comment：** 这里是生产 gateway 和玩具 proxy 的差异：生命周期不是函数返回就结束，尤其是 streaming response。
...
**Explain：** 这段结构定义了 RouterManager 的核心职责：它不是一个具体转发器，而是多个 `RouterTrait` 实例的选择器。
...
**Comment：** RouterManager 是 gateway 的“路由路由器”：先选哪类 router，再由具体 router 选 worker。
...
**Explain：** 这段很短，但语义明确：liveness 只回答“gateway 进程是否还能响应”。
...
**Comment：** 读运维相关代码时要先看语义边界。这里的“少做事”是设计选择。
...
**Explain：** `flush_cache` 是控制面广播型操作，不走普通请求选 worker policy。
...
**Comment：** 管理 endpoint 的核心不是“请求路由”，而是“对拓扑执行一致操作”。
...
**Explain：** 这两个 endpoint 是 gateway 本地能力，不是 inference worker 代理。
...
**Comment：** 这类 endpoint 提醒读者：gateway 不只是 L7 proxy，也承载少量协议适配和后处理能力。
...
**Explain：** 这段说明 service discovery 是可选后台拓扑来源，和手工 worker 管理 API 可以并存。
...
**Comment：** Service discovery 是 `WorkerRegistry` 的输入源之一，不改变请求路由主线。
...
**Explain：** Round robin 是最简单 policy，也能看出 policy 的边界：它只接收候选 worker 和 `SelectWorkerInfo`，不直接访问 registry。
...
**Comment：** policy 的设计目标是可替换。Round robin 不是生产最优，但它定义了其他策略应该遵守的接口形状。
...
**Explain：** 这段不是路由逻辑，但体现了 worker 访问的基础性能假设：HTTP client 是可复用资源。
...
**Comment：** worker 抽象的底层资源选择会影响整个 gateway 的吞吐和尾延迟。不要把它看成普通工具变量。
