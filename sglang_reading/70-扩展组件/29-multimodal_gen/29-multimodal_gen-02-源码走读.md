...
**Explain：** 启动器的设计是“主进程负责生命周期，worker 负责模型”。父进程不加载模型，只协调 pipe、ready 握手和 HTTP 入口。
...
**Comment：** 这里的“scheduler”不是单独线程，而是每个 GPU worker 内部的 scheduler event loop。
...
**Explain：** 这个握手把可服务状态定义为“所有 worker 都已完成 scheduler 初始化”。HTTP 只有在这个条件满足后才会 bind。
...
**Comment：** 这个设计也让部署系统更容易判断健康：进程启动失败会直接表现为服务未监听，而不是运行时随机 500。
...
**Explain：** 端口分配保持小函数化，是因为 monolithic、pool disagg、standalone role 都需要类似逻辑，但各自起始偏移不同。
...
**Comment：** 这类小工具解释了为什么启动器不把端口写死：多角色 diffusion serving 的 socket 数量比普通单 HTTP 服务多。
...
**Explain：** 这里的设计哲学是“角色池先描述拓扑，再启动进程”。日志中的 encoder/denoiser/decoder 数量是后续 endpoint 分配和调度策略的基准。
...
**Comment：** 这一小段日志不是装饰，它是多角色服务启动时最早可见的拓扑声明。
...
**Explain：** 这是服务启动器的最后一道边界：出错时优先清理整个进程树，而不是依赖 Python 对 daemon process 的隐式回收。
...
**Comment：** `.obsidian` 或阅读笔记侧不应模仿这种清理逻辑；它只属于运行时进程管理。
...
**Explain：** ready 的语义是 scheduler 已经成功构造，而不是 worker 进程刚启动。这样父进程等待的是可接收请求的后端，而不是一个还在加载模型的进程。
...
**Comment：** rank0 和 slave rank 的差异不在这个入口函数本身，而在传入 scheduler 的 pipe 列表和 scheduler 内部事件循环。
...
**Explain：** 这让 GPUWorker 对外呈现为“构造完成即可执行”的对象，而不是让 scheduler 在每次请求前检查模型和通信组是否就绪。
...
**Comment：** CPU group 的存在说明这里不仅服务 GPU collective，也考虑跨进程 CPU-side 控制消息。
...
**Explain：** 这些 import 暗示 GPUWorker 不是一个普通单卡执行器，而是模型并行环境的一部分。它需要能设置进程标题、构建 group、并按并行 rank 执行 pipeline。
...
**Comment：** 阅读这部分时不要把 `num_gpus` 等同于纯 data parallel；这里的 GPU 可能被多个并行维度切分。
...
**Explain：** 这里没有吞掉 OOM，而是“补充诊断再失败”。这保证启动器的 ready barrier 不会把 OOM worker 当成可用。
...
**Comment：** 这段和 ready 握手是一组设计：worker 失败要清楚地失败，而不是降级成半启动服务。
...
**Explain：** HTTP 层不直接持有 GPU worker，只持有 scheduler client。这样 HTTP API、offline broker 和 Vertex route 都走同一条 backend 转发路径。
...
**Comment：** broker 和 HTTP route 是两个入口，但它们都不绕过 scheduler，这保持了请求调度的一致性。
...
**Explain：** 这是一个启动闭环：HTTP 先起来，warmup task 再通过真实 HTTP 入口验证端到端路径，而不是直接调用内部函数。
...
**Comment：** 这解释了为什么 warmup bypass path 需要存在：health route 必须在 warmup 期间仍能被访问。
...
**Explain：** 这是一种“对用户请求关门，对控制面开门”的启动策略：生成请求等待 warmup，健康和发现接口仍可返回。
...
**Comment：** 这段很短，但直接决定启动期外部负载均衡看到的服务状态。
...
**Explain：** 同步 client 是单 socket、单状态机模式，适合命令式调用；它和 FastAPI 的 async client 分开，避免并发 HTTP 请求共享同一个 REQ socket。
...
**Comment：** FastAPI 使用 async client 的“每请求一个 socket”策略，正是为了避开这个同步 client 的并发限制。
...
**Explain：** 这不是普通 import，而是 pipeline 执行层的契约：请求在 stage 间流动时仍是 `Req`，跨出 executor 后统一落到 `OutputBatch`。
...
**Comment：** 读 executor 时先抓住 `Req`/`OutputBatch`，比先看具体 pipeline stage 更容易理解控制流。
...
**Explain：** 这里把“怎么测”和“怎么跑 stage”分离。子类只需要实现 stage 调度，profiling 和推理上下文由基类包住。
...
**Comment：** 下面的 stage hook 正是为了处理 offload/FSDP 等需要临时放宽 inference mode 的特殊情况。
...
**Explain：** 这让 stage 专注“如何计算”，executor 专注“何时加载组件、何时打 profiling 标记、何时切上下文”。
...
**Comment：** 这也是为什么 pipeline executor 是抽象基类：不同执行器可以改 stage 顺序，但共享 hook 语义。
...
**Explain：** 输出 materialization 是 API 边界的一部分，和模型 forward 不同。把它放在 entrypoints utils，worker 只编排何时调用。
...
**Comment：** 这解释了为什么 GPUWorker 看起来也有“entrypoint”逻辑：它负责把模型输出变成可传输响应。
...
**Explain：** session 释放被设计成普通 `OutputBatch` 响应，而不是 side-effect-only 方法，这样 scheduler/client 路径可以统一返回结果。
...
**Comment：** `max_sessions=1` 的 cache 初始化说明当前 worker 更偏向单实时连接管理，而不是无限 session 池。
