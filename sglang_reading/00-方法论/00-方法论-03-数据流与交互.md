...
**Comment：** 此后进入 FastAPI/uvicorn 与 TokenizerManager 进程树（[[03-HTTP-Server-00-MOC|HTTP Server]]、[[06-TokenizerManager-00-MOC|TokenizerManager]]）。本模块数据流在 **HTTP 监听启动前** 结束；请求进入 GPU 的路径见 [[全链路请求追踪]]。
