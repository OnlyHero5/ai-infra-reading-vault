...
> **前置批次：** [[04-Arguments-TrainRollout-00-MOC]]  
> **后续批次：** [[06-PlacementGroup-00-MOC]]

## 本批目标

读者**不打开 `slime/`**，仅读本批六件套，应能：
...
本批是阶段 I（批次 02–05）的**收官批**。完成本批后，应能画出从 CLI 到 RL 主循环的完整栈——数据准备批负责栈**左侧**的权重形态，训练批负责**右侧**的运行时编排。

```mermaid
flowchart TB
    subgraph prep["数据准备（本批 05）"]
    ...
    subgraph entry["train.py 入口（批次 02）"]
        PA["parse_args()<br/>slime/utils/arguments.py"]
        TR["train(args)"]
        PA --> TR
    end

    subgraph bootstrap["启动编排（批次 02 + 06）"]
...
**Explain：** 阶段 I 验收要求把「参数解析 → 资源分配 → 模型创建 → generate → train → update_weights」串成一条线。本批补齐**训练前**一步：`MODEL_ARGS` + `convert_hf_to_torch_dist.py` 产出 `--ref-load` 指向的 Megatron 权重；`--hf-checkpoint` 仍保留给 tokenizer 与 SGLang 侧 HF 元数据。
...
- `train()` 内 bootstrap 与主循环见 [[02-训练主循环-00-MOC]]；本批 MOC 图与之衔接
- 训练中途保存的 Megatron checkpoint 可用 `convert_torch_dist_to_hf.py` 导出，供 Hugging Face 生态或 `--update-weight-from-disk` 路径使用（批次 25 详述）
...
- 本批完成后为 **图谱增量点**（批次 05 / 10 / 15 …）：运行 `/understand --language zh` + `/understand-domain`
- 内嵌代码基线 commit：`22cdc6e1`
