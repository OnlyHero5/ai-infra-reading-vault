...
**Explain：** Slime **trainer-side** delta（本批）在 host apply 成 full HF 再 reload。SGLang 原生 `load_format=delta`（external PD 测试）是 **引擎侧** 读 delta 文件——路径不同，不要混淆 `UpdateWeightFromDiskDelta` 与 engine payload `load_format=delta`。
