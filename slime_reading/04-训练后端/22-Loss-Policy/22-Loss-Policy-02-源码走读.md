...
|------|----------|-------------------|----------|
| 1 | [[#1. ppo_utils — PPO clip]] | `ratio`、dual-clip 三分支如何算 `pg_loss`？ | 5 |
| 2 | [[#5. policy_loss_function — logprob 重算]] | 训练步如何重算 logprob、`old_log_probs` 从哪来？ | 5 |
| 3 | [[#7. KL 分支与 CISPO / PPO 选择]] | GSPO vs 普通 KL、`ppo_kl` 如何喂入 clip/CISPO？ | 6 |
| 4 | [[#8. TIS — vanilla_tis_function]] · [[#9. TIS 路径 reducer 重建]] | rollout/train logprob 不匹配时 importance weight 怎么作用？ | 5 |
| 5 | [[#10. entropy、KL loss、metrics]] | 最终 loss 标量由哪些项组成、如何 reduce？ | 5 |
| 6 | [[#14. loss_function — dispatch 与 allgather-CP 零 loss]] | Megatron `forward_step` 如何 dispatch 并 rescale loss？ | 4 |

**跳过策略：** §2 CISPO 公式、§3–4 GSPO/`compute_approx_kl`、§11–13 value/sft/icepop、§16–26 边缘专题（OPSM mask、GRPO advantage 等）留二遍按需跳读。
...
```python
## 来源：slime/utils/ppo_utils.py L604-L609
        else:
