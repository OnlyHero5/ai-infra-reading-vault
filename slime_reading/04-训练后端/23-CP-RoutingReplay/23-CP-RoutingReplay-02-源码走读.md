...
```python
## 来源：slime/backends/megatron_utils/cp_utils.py L228-L229
            else:
                reduced[key] = sum(values) / dp_size
```
...
```python
## 来源：slime/utils/routing_replay.py L43-L54
...
```python
## 来源：slime/backends/megatron_utils/model.py L576-L638（train_one_step.forward_step 闭包）
        batch = get_batch(
            data_iterator,
