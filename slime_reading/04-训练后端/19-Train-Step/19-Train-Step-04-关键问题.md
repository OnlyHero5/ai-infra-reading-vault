...
真正「generate 与 train 重叠」见 `train_async.py` 与批次 14 fully-async，与本模块 `train()` 实现正交。
...
注意：`update_weights` 另有独立的 wake/reload 逻辑（见批次 24）。
...
无 Critic 时走单 Actor `async_train`；`advantage_estimator=gspo` 等会禁用 log-prob 复用优化。GRPO 细节见批次 21–22，Train Step 层调用栈相同，差异在 `compute_advantages_and_returns` 与 `loss_function` 分支。
