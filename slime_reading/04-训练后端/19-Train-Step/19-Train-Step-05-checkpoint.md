...
> 完成本批后逐项自测。全部打勾再更新 [[Slime-progress]] 批次 19 为 ✅。

---

## 验收清单

- [ ] 不打开 `slime/` 目录，仅读本批 slime_reading 文档，能说明 **Train Step** 模块职责（一次 rollout 内 Megatron 训练：数据反序列化 → forward/backward → 权重备份）
...
→ [[20-Train-Data-00-MOC]]：`process_rollout_data`、`get_data_iterator`、`get_batch` 如何构造本批消费的 `rollout_data` 字段。
