...
| 对照源码 | `sglang/`、`slime/`、`flash-attn/` upstream（读者可选） |
| 关系维度 | 数据流 / 调用链 / 专题依赖 |
...
| `00-导读与总览-00-MOC.md` | SGLang 导读入口 |
...
Slime 批次将 `sglang/` 前缀换为 `slime/`；FlashAttention 专题将前缀换为 `flash-attn/`：
...
- 修改任何正文笔记前，必须先完整阅读该笔记引用到的 upstream 源码文件，而不是只看已有笔记或摘录片段。
- 写“为什么这样设计”时，必须能回指到源码中的初始化顺序、调用链、分支条件、数据结构约束、测试或文档；不能凭经验补设计动机。
- 源码走读段落采用**设计解释型 ETC**：`问题与约束` → `设计选择` → `Explain` → `Code` → `代码逻辑` → `为什么这样写` → `不变量与失败模式` → `Comment`。
- `Comment` 不只列字段含义，应把代码放回专题主线，说明读者应抓住的设计取舍。
- 新增或调整源码摘录后，运行 `node 90_meta/audit_source_evidence.mjs`，确保 `来源：路径 Lx-Ly` 能定位到当前 upstream 基线。
...
迁移已完成：**全库使用双链**，相对路径仅保留在 `_TEMPLATE/*模板说明.md` 说明中。
...
```markdown
> [!tip] 调试提示
> Prefill 卡住时先查 [[sglang_reading/30-请求调度/07-Scheduler/07-Scheduler-04-关键问题|Scheduler 关键问题]]。
```
...
| `90_meta/audit_source_evidence.mjs` | 源码引用路径与行号扫描 |
